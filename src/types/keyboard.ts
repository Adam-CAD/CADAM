...
export const matchesShortcut = (
  ...
  // Check modifiers
  const hasModKey = modifiers.includes('mod')
    ? isMac()
      ? event.metaKey
      : event.ctrlKey
    : false;
  const hasShift = modifiers.includes('shift') ? event.shiftKey : false;
  const hasAlt = modifiers.includes('alt') ? event.altKey : false;
